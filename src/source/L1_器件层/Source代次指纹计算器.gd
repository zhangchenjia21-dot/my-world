...
func calculate(manifest: Dictionary, referenced_files: Array) -> Dictionary:
	...
	var schema := String(manifest.get("schema_version", ""))
	var separator := "MY_WORLD_SOURCE_V0.2" if schema.ends_with(".v0.2") else "MY_WORLD_EXPANSION_V0.1" if schema == Rules.EXPANSION_SCHEMA else "MY_WORLD_SOURCE_V0.1"
