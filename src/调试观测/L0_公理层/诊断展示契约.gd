...
const CAPACITY := 64
const LANES := {"narrative": "叙事", "world": "世界语义", "identity": "人物身份", "character": "角色", "experiences": "重要经历", "people": "人物卡", "threads": "事务", "mechanics": "公开判定", "inventory": "行囊", "recommendations": "推荐行动", "save": "存档", "restore": "恢复进度"}
...
const REASONS := {"configuration_failure":"模型配置不可用，未执行恢复重试","invalid_inventory":"物品变更结构或引用无效，未应用该部分","check_accepted":"公开检定已接受", "no_check_accepted":"无需掷骰已接受", "already_accepted":"复用已接受的机制结果", "degraded":"判定控制不可用，已按普通叙事继续（无检定）","already_current": "已处于所选进度", "historical_skipped": "历史回合未重新处理", "already_materialized": "已复用当前历史结果", "ready": "", "committed": "", "initial_committed": "", "accepted": "", "saved": "已保存当前进度", "restored": "已切换进度，旧诊断已清除", "input_unavailable": "当前没有合法推荐机会", "malformed_response": "模型响应结构无效", "response_oversized": "模型响应超过大小限制", "provider_failure": "模型服务调用失败", "timeout": "请求超时", "cancelled": "请求已取消", "stale": "当前回合已被替换，旧结果已丢弃", "persistence_failure": "持久化失败", "initial_profile_unavailable": "初始角色材料暂不可用", "input_oversized": "输入材料超过大小限制", "unknown_failure": "处理未完成，请稍后重试", "backup_warning": "存档已保存，安全备份刷新失败"}

static func reason(code: String) -> String:
	if REASONS.has(code): return code
	if code in ["provider_start_failure", "analysis_start_failure", "start_failed", "analysis_provider_failure", "missing_key", "transport", "http"]: return "provider_failure"
...
static func row(lane: String, index: int, terminal: String, change: String, code: String, counts: Dictionary = {}, elapsed: int = -1) -> Dictionary:
	...
	var safe_counts := {}
	for key: String in ["changes", "knowledge", "actors", "bindings", "added", "removed", "updated", "total", "checks", "no_checks", "attempt"]:
