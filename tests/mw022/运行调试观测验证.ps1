...
$runName = 'mw022-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw022/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = ,@('mw022', 'tests/mw022/会话调试观测纵向测试.gd') }
    'Window' { $cases = ,@('mw022-ui', 'tests/mw022/会话调试窗口测试.gd') }
    'Regressions' { $cases = @(
...
try {
    foreach ($case in $cases) {
        ...
        [string[]]$displayArgs = if ($Mode -eq 'Window') { @() } else { @('--headless') }
