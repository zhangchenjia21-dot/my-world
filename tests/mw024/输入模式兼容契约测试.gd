...
func _run() -> void:
	...
	check(messages[0].content.contains("Active Input Mode: ooc") and messages[-1].content.contains("GM Guidance"), "active OOC request structural marker")
	...
	check(JSON.stringify(messages).contains("GM Guidance") and not messages[0].content.contains("Active Input Mode: ooc") and messages[-1].content == "我继续走。", "subsequent action sees recent OOC as guidance")
