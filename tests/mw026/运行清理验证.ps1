...
$runName = 'mw026-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw026/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw026-focused','tests/mw026/公开机制上下文测试.gd')) }
    'Window' { $cases = ,@('mw019-mw026-window','tests/mw026/紧凑推荐窗口测试.gd') }
    'Regressions' { $cases = @(
