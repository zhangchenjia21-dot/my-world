...
if (-not $Root) { $Root = Join-Path $projectRoot ('build/mw019/real-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
...
try {
    ...
    & $Godot --headless --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/mw019/真实推荐验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "MW-019 real vertical failed with exit code $exitCode" }
Write-Host 'MW-019 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
