...
if (-not $Root) { $Root = Join-Path $projectRoot ('build/mw018/real-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
$Root = [IO.Path]::GetFullPath($Root)
if (-not $Root.StartsWith(($projectRoot + '\build\mw018\'), [StringComparison]::OrdinalIgnoreCase)) { throw 'Use task-owned build/mw018 root.' }
...
if ($exitCode -ne 0) { throw "MW-018 real vertical failed with exit code $exitCode" }
Write-Host 'MW-018 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
