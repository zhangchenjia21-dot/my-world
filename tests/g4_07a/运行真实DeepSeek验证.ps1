...
param(
    ...
    [string]$Root = 'D:\AI\Projects\my-world\build\g4_07a_real_provider'
)
...
try {
    ...
    & $Godot --headless --path $projectRoot --script 'res://tests/g4_07a/真实DeepSeek首次开场测试.gd' -- "--root=$($Root.Replace('\', '/'))" "--evidence=$($evidence.Replace('\', '/'))"
    if ($LASTEXITCODE -ne 0) { throw "G4-07A real Provider validation failed with exit code $LASTEXITCODE" }
}
finally {
