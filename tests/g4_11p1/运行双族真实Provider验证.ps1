...
param(
    ...
    [string]$Root = 'D:\AI\Projects\my-world\build\g411'
)

# 只把仓库现有 credential 白名单注入子进程；不打印、不复制、不持久化 secret value。
# Owner Source/Games/settings/current DB 仅做前后内容 fingerprint，所有可变验证状态均在 $Root。
...
function Get-PathFingerprint([string]$Path) {
    ...
    return @{ kind = 'directory'; count = $files.Count; sha256 = $digest }
}
...
$evidencePath = Join-Path $Root 'two-family-reality.json'
...
try {
    ...
    & $Godot --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/g4_11p1/双族现实准备验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))" "--evidence=$($evidencePath.Replace('\', '/'))"
...
if (($before | ConvertTo-Json -Depth 6 -Compress) -ne ($after | ConvertTo-Json -Depth 6 -Compress)) {
    throw 'Owner production Source/Games/settings/current DB fingerprint changed.'
}
if ($exitCode -ne 0) { throw "G4-11P1 real vertical failed with exit code $exitCode" }
Write-Host 'G4-11P1 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
