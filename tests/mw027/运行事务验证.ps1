...
$runName = 'mw027-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw027/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw027-focused','tests/mw027/事务纵向验证.gd')) }
    'Window' { $cases = ,@('mw027-window','tests/mw027/事务纵向验证.gd') }
    'Regressions' { $cases = @(
