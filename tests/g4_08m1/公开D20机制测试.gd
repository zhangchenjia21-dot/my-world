...
func _test_retry_restart_and_no_check() -> void:
	...
	no_check_stub.simulate_delta("侍从立即答出今日日期。")
	no_check_stub.simulate_completed()
	_check(no_check_stub.requests.size() == 2 and no_check_rng.invocation_count == 0, "D/G NO_CHECK decouples control/narrative with no RNG/check")
