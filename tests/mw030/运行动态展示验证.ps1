...
$runName = 'mw030-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw030/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw030-focused','tests/mw030/动态展示纵向验证.gd')) }
    'Window' { $cases = ,@('mw030-window','tests/mw030/动态展示纵向验证.gd') }
    'Regressions' { $cases = @(
