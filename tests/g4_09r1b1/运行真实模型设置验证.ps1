...
$taskRoot = Join-Path $projectRoot 'build\g4_09r1b1\real-settings'
$evidence = Join-Path $taskRoot 'real-settings-evidence.json'
...
try {
    ...
    $shotsArg = (Join-Path $taskRoot 'shots').Replace([char]92, [char]47)
    $evidenceArg = $evidence.Replace('\', '/')
    & $Godot --path $projectRoot --log-file (Join-Path $taskRoot 'godot.log') --script 'res://tests/g4_09r1b1/真实模型设置界面验证.gd' -- "--root=$rootArg" "--shot-dir=$shotsArg" "--evidence=$evidenceArg"
    exit $LASTEXITCODE
}
finally {
    foreach ($name in @('DEEPSEEK_API_KEY', 'KIMI_API_KEY')) {
