...
if (-not $Root) { $Root = Join-Path $projectRoot ('build/mw017/real-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
$Root = [IO.Path]::GetFullPath($Root)
if (-not $Root.StartsWith(($projectRoot + '\build\mw017\'), [StringComparison]::OrdinalIgnoreCase)) { throw 'Use task-owned build/mw017 root.' }
if (Test-Path -LiteralPath $Root) { throw 'Use a fresh evidence root.' }
$envFile = 'D:/AI/Projects/my-world/.env.local'
if (-not (Test-Path -LiteralPath $envFile -PathType Leaf)) { throw '.env.local is missing.' }

$allowed = @('DEEPSEEK_API_KEY', 'KIMI_API_KEY')
$values = @{}
...
try {
    ...
    & $Godot --headless --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/mw017/真实Provider身份桥验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "MW-017 real vertical failed with exit code $exitCode" }
Write-Host 'MW-017 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
