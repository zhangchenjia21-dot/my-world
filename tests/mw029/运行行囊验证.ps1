...
$runName = 'mw029-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw029/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw029-focused','tests/mw029/行囊纵向验证.gd')) }
    'Window' { $cases = ,@('mw029-window','tests/mw029/行囊纵向验证.gd') }
    'Regressions' { $cases = @(
