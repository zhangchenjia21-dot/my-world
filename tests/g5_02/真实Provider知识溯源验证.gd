...
## G5-02M1 真实 selected-Provider 知识溯源验证 —— task-owned root 持久化；
## 证据只记录 profile、hash、计数与已提交 provenance，不保存完整请求/响应/reasoning/credential。
## 一次 bounded attempt：Player + Guaranteed NPC 的私有信息行动，验证组合语义响应与
## durable 知识 provenance 及后续 Context 的 actor-knowledge 边界。
...
func _run() -> void:
	...
	if _task_root.find("g502") < 0 or _evidence_path.is_empty():
		return _finish_with_failure("必须提供 task-owned g502 root/evidence")
	...
	creation.set_settings("G5-02M1 真实知识溯源", "Narrative", "从赤壁前夕刘备军营的现实压力自然开始。")
	var created := FinalCreate.new(library, _task_root.path_join("creation"), _task_root.path_join("library"), _task_root.path_join("games")).create_or_resume("g5-02-real-han", creation.composition_snapshot())
	...
	# 一次真实普通行动：Player 单独查看私有记录。
	var action := "我独自查看案上刚送来的北门守军换防记录，记下关键时辰。"
...
func _play_real_turn(action: String) -> Dictionary:
	...
	return {
		...
		"knowledge_dropped": int(semantic.get("knowledge_dropped", 0)),
	}
...
func _check(condition: bool, label: String) -> void:
	if condition:
		print("G5-02 REAL PASS | %s" % label)
	else:
		_failures += 1
		push_error("G5-02 REAL FAIL | %s" % label)
...
func _finish() -> void:
	_write_evidence()
	print("G5-02 REAL | done failures=%d" % _failures)
