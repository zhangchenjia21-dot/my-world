...
param(
    ...
    [string]$Root = 'D:\AI\Projects\my-world\build\g502'
)
...
function Get-PathFingerprint([string]$Path) {
    ...
    try {
        ...
        return @{ kind = 'unreadable'; count = 0; sha256 = '' }
    }
...
try {
    ...
    & $Godot --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/g5_02/真实Provider知识溯源验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))" "--evidence=$($evidencePath.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "G5-02M1 real vertical failed with exit code $exitCode" }
Write-Host 'G5-02M1 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
