...
param(
    ...
    [string]$Root = 'D:\AI\Projects\my-world\build\g501'
)
...
try {
    ...
    & $Godot --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/g5_01/真实Provider世界回合验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))" "--evidence=$($evidencePath.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "G5-01M1 real vertical failed with exit code $exitCode" }
Write-Host 'G5-01M1 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
