...
# 隔离 fixture 不参与 Godot 导入，避免扫描长路径的测试 Source 副本。
$ignore = Join-Path $projectRoot 'build/.gdignore'
if (-not (Test-Path -LiteralPath (Split-Path $ignore))) { New-Item -ItemType Directory -Path (Split-Path $ignore) | Out-Null }
if (-not (Test-Path -LiteralPath $ignore)) { New-Item -ItemType File -Path $ignore | Out-Null }
$runName = 'mw033-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw033/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw033-focused','tests/mw033/叙事工作集纵向测试.gd')) }
    'Window' { $cases = ,@('mw032-window','tests/mw032/现实门窗口验证.gd') }
    'Regressions' { $cases = @(
        @('mw032','tests/mw032/现实门修正纵向测试.gd'),
        @('mw020','tests/mw020/上下文预算核算测试.gd'),
        @('g4_07a','tests/g4_07a/首次开场运行时聚焦测试.gd'),
        @('g4_07b','tests/g4_07b/可玩界面整合测试.gd'),
        @('mw030','tests/mw030/动态展示纵向验证.gd'),
        @('mw029','tests/mw029/行囊纵向验证.gd'),
        @('mw028','tests/mw028/系统机制纵向验证.gd'),
        @('mw027','tests/mw027/事务纵向验证.gd'),
        @('mw026-focused','tests/mw026/公开机制上下文测试.gd'),
        @('mw025-focused','tests/mw025/角色推荐反馈纵向测试.gd'),
        @('mw024-contract','tests/mw024/输入模式兼容契约测试.gd'),
        @('mw024-vertical','tests/mw024/场外指导纵向测试.gd'),
...
try {
    foreach ($case in $cases) {
        ...
        & $Godot @displayArgs --path $projectRoot --quit-after 6000 --script ('res://' + $case[1]) -- @userArgs *> $log
        $code = $LASTEXITCODE
