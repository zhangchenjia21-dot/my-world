...
param(
    ...
    [string]$Root = 'D:\AI\Projects\.worktrees\my-world\mw-014\build\mw014\real'
)
...
try {
    ...
    & $Godot --headless --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/mw014/真实Provider信息整理验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))" "--evidence=$($evidencePath.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "MW-014 real vertical failed with exit code $exitCode" }
Write-Host 'MW-014 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
