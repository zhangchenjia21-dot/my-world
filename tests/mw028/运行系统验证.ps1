...
$runName = 'mw028-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw028/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw028-focused','tests/mw028/系统机制纵向验证.gd')) }
    'Window' { $cases = ,@('mw028-window','tests/mw028/系统机制纵向验证.gd') }
    'Regressions' { $cases = @(
