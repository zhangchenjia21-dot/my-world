...
func _run_wizard_at_size(shell: Variant, label: String) -> void:
	...
	await _next(wizard)
	_check(wizard.step == 6 and wizard.review_text.visible and wizard.create_placeholder_button.disabled, "%s Review reachable and Final Create disabled" % label)
	_check(_inside_window(wizard.cancel_button.get_global_rect()) and _inside_window(wizard.create_placeholder_button.get_global_rect()), "%s Review navigation remains inside viewport" % label)
