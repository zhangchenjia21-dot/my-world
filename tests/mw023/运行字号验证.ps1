...
$runName = 'mw023-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw023/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = ,@('mw023', 'tests/mw023/游戏字号窗口验证.gd') }
    'Window' { $cases = ,@('mw023-ui', 'tests/mw023/游戏字号窗口验证.gd') }
    'Regressions' { $cases = @(
