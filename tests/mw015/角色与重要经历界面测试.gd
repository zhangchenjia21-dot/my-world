...
## 真实 FinalCreate Game + real Shell(main.tscn) + real SQLite：右侧 概览|角色|重要经历|存档
...
func _test_shell_surfaces() -> void:
	...
	# 1 导航结构：恰好 概览|角色|重要经历|存档 四个互斥 toggle，默认概览。
	...
	_check(nav_labels == PackedStringArray(["概览", "角色", "重要经历", "存档"]) and all_toggle, "1 right navigation is exactly 概览|角色|重要经历|存档 toggles")
