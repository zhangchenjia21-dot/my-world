...
if (-not $Root) { $Root = Join-Path $projectRoot ('build/mw015/r1/real-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
...
try {
    ...
    & $Godot --headless --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/mw015/真实Provider稀疏经历验证.gd' -- @caseArgs
...
if ($exitCode -ne 0) { throw "MW-015 R1 real semantic validation failed with exit code $exitCode" }
Write-Host 'MW-015 R1 REAL expected semantic outputs | Owner production fingerprints unchanged.'
