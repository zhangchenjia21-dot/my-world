...
param(
    ...
    [string]$Root = 'D:\AI\Projects\.worktrees\my-world\mw-015-r2\build\mw015r2\real'
)
...
$envFile = 'D:/AI/Projects/my-world/.env.local'
...
if (-not $Root.StartsWith(($projectRoot + '\build\mw015r2\'), [StringComparison]::OrdinalIgnoreCase)) { throw 'Real smoke must stay inside task build/mw015r2.' }
if (Test-Path -LiteralPath $Root) { throw 'Use a fresh root; prior evidence is preserved.' }
...
$evidencePath = Join-Path $Root 'curation-real.json'
...
try {
    ...
    & $Godot --headless --path $projectRoot --log-file (Join-Path $Root 'godot.log') --script 'res://tests/mw015r2/真实Provider初始角色验证.gd' -- "--root=$($stateRoot.Replace('\', '/'))" "--evidence=$($evidencePath.Replace('\', '/'))"
...
if ($exitCode -ne 0) { throw "MW-015 R2 real vertical failed with exit code $exitCode" }
Write-Host 'MW-015 R2 REAL PROVIDER PASS | Owner production fingerprints unchanged.'
