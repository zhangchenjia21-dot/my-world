...
if ([string]::IsNullOrEmpty($Root)) { $Root = Join-Path $projectRoot ('build/mw015r2/verify-' + (Get-Date -Format 'yyyyMMdd-HHmmss')) }
$Root = [IO.Path]::GetFullPath($Root)
if (-not $Root.StartsWith(($projectRoot + '\build\mw015r2\'), [StringComparison]::OrdinalIgnoreCase)) { throw 'Verification root must stay in this worktree build/mw015r2.' }
...
# R2 初始 lane / Shell 专项通过后运行 Task Packet 指定回归和导出。
# real Provider calls = 0。G4-07B Narrative 关键路径单独验证，既有退出告警见报告。

$cases = @(
    @('initial', 'tests/mw015r2/初始角色基线纵向测试.gd', 'mw015r2'),
    @('initial-ui', 'tests/mw015r2/初始角色界面纵向测试.gd', 'mw015r2-ui'),
    @('contract', 'tests/mw015r2/初始整理契约缺口验证.gd', 'mw015r2-contract'),
...
try {
    foreach ($case in $cases) {
        ...
        if ($case[0] -eq 'initial-ui') {
            $extraArgs += '--model-evidence=res://docs/mw015/r2/evidence/real-initial-attempt-02.json'
        }
