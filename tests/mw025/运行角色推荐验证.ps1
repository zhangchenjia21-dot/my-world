...
$runName = 'mw025-' + (Get-Date -Format 'HHmmss')
$evidenceRoot = Join-Path $projectRoot ('build/mw025/' + $runName)
New-Item -ItemType Directory $evidenceRoot | Out-Null
switch ($Mode) {
    'Focused' { $cases = (,@('mw025-focused','tests/mw025/角色推荐反馈纵向测试.gd')) }
    'Window' { $cases = ,@('mw025-window','tests/mw025/角色推荐反馈纵向测试.gd') }
    'Regressions' { $cases = @(
